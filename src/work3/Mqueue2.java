	...
	public class Mqueue2 {
		...
		public static void display (RR3 vm)
		{
			...
			while (i<25)
			{
				...
				{
				c++;
				avF = avF + vm.finishTime.get(i)  - cloudletList.get(i).getStartTime();
			...
			for (int i = 0; i < size; i++) {
				...
				if (cloudlet.getCloudletStatus() == Cloudlet.SUCCESS){
					...
					Log.printLine( indent + indent + cloudlet.getResourceId() + indent + indent + cloudlet.getVmId() +
							...
							indent + indent + dft.format(finishTime.get(cloudlet.getCloudletId()))+ indent + indent + cloudlet.getSubmissionTime());
				}
			}
